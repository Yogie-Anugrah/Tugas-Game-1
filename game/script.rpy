...
# Ren'Py 8.4.1-ready: HUD + Timed Choice + Countdown Bar
# Catatan: pakai screen language resmi (bar/value/timer). Lihat docs screens/timer/values. 
...
# ==== GLOBAL STATE ====
default Heat = 0                # 0..100
default VoiceHP = 3             # 0..3
default TRUST_VERSO = 0         # -1..+2
...
# ==== HUD SCREEN ====
# Pakai VariableValue() untuk mengikat bar ke variabel store (docs: screen actions/values).
screen hud():
    zorder 100
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    # TRUST max 2 buat visual (0..2)
                    bar value VariableValue("TRUST_VERSO", 2) xmaximum 360

# ==== TIMED CHOICE OVERLAY (COUNTDOWN BAR + TEXT) ====
# Resmi: screen+timer+Jump (docs screens & timer). Countdown update via SetScreenVariable. 
# Bar pakai StaticValue(remaining, seconds) supaya non-interaktif, tapi tetap realtime saat 'remaining' berubah.
screen choice_timer(seconds=10.0, timeout_label="timeout"):
    ...
    frame style "timer_frame":
        ...
        # progress bar (kiri penuh -> kanan habis). Kita tampilkan waktu tersisa; range = seconds.
        bar value StaticValue(remaining, seconds) xmaximum 520

# ==== SMALL HELPERS (hindari nama awalan '_', reserved by Ren'Py) ====
...
label start:
    ...
    # Timed menu: 15 detik; kalau telat -> prolog_timeout.
    show screen choice_timer(15.0, "prolog_timeout")
...
label climax_stairs:
    ...
    # Timed decision at climax (8 detik). Timeout -> branch_bad_check (panik).
    show screen choice_timer(8.0, "branch_bad_check")
...
label branch_bad_check:
    if VoiceHP <= 0 or (not VENT_LOCK and not TANK_ON):
        ...
        jump ending_bad  # desain: tanpa mitigasi penuh, tetap buruk
